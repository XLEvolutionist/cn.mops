...
# set the wd
setwd("/Users/simonrenny-byfield/CNV_PAV/output/gene23")
#setwd(args[1])
...
subsetByOverlaps(GRgenes,cn.mopsRanges,ignore.strand = TRUE, minoverlap=1, type="any")
...
totalOverlap<-subsetByOverlaps(CNgenes,SWgenes,ignore.strand = TRUE, minoverlap=1, type="any")

# grab the sw genes with CNV calls in TIL01
swTIL<-SWgenes[SWgenes$value != 0]
# grab the cn.mops genes with calls in TIL01
cnTIL<-CNgenes[CNgenes$filtered_TIL01_sorted.bam !=0]
...
subsetByOverlaps(cnTIL,swTIL,ignore.strand = TRUE, minoverlap=1, type="any")
...
upDown.df<-cbind("sw"=swTIL$value[idx.compare[,1]],"cn.mops"=cnTIL$filtered_TIL01_sorted.bam[idx.compare[,2]])

length(upDown.df[,1][upDown.df[,1] == upDown.df[,2]])
#127/168 of the overlaps 76% of them agree in their call.
...
upCNVs.df<-cbind("sw"=swTIL$value[swTIL$value == 1][up.idx[,1]],"cn.mops"=cnTIL$filtered_TIL01_sorted.bam[up.idx[,2]])
agreeUp<-length(upCNVs.df[,1][upCNVs.df[,1] == upCNVs.df[,2]])

# some important vairable
totalUp<-dim(upCNVs.df)[1]
...
downCNVs.df<-cbind("sw"=swTIL$value[down.idx[,1]],"cn.mops"=cnTIL$filtered_TIL01_sorted.bam[down.idx[,2]])
agreeDown<-length(downCNVs.df[,1][downCNVs.df[,1] == downCNVs.df[,2]])

totalDown<-dim(downCNVs.df)[1]
...
subsetByOverlaps(cnTIL,swNegTIL,ignore.strand = TRUE, minoverlap=1, type="any")
# so a 345/2342 or ~14.7% of negative calls in sw have a call in cn.mops data.

# how many of these are down CNVs
falseDown<-subsetByOverlaps(swNegTIL,cnTIL[cnTIL$filtered_TIL01_sorted.bam == -1],ignore.strand = TRUE, minoverlap=1, type="any")
falseUp<-subsetByOverlaps(swNegTIL,cnTIL[cnTIL$filtered_TIL01_sorted.bam == 1],ignore.strand = TRUE, minoverlap=1, type="any")
###
