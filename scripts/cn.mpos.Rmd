...
**Alternatively use .bam files that have been filtered using MINMAPQ **
...
This produced zero overlaps between the chip data and the CNV calls (for TIL01). In response I changed the filtering to `-MINMAPQ=30`.

** Changing window size for cn.mops runs **

When loading in the bam files there is an option to use window size. As far as I can tell the function then reports average coverage in non-overlapping windows across the entire genome. The actual command looks like this:

```
bamDataRanges <- getReadCountsFromBAM(BAMFiles, refSeqName=i,  mode="paired",WL=len)
```
Where `len` is your desired window lenght.

** CNVs over genic regions **

There is a `exomecn.mops` function that will assess CNVs over genic regions. In order to run this analysis you need to convert a BED file to a `GRanges` object, and then suuply the `GRanges` obejct to the function. I have modified one of the scripts to perfomr this:
```
# Simon Renny-Byfield
# 16/12/2014
